...
import { renderCustomers, openCustomerModal, saveCustomer, openCustomerProfileModal, openGiveModal, processGive, openReceiveModal, processReceive } from './modules/customer.js';
...
window.forgotPassword = async (e) => {
    ...
    try {
        ...
        alert("If account exists, link sent.");
    }
};
...
function startListeners() {
    ...
    listeners.push(onSnapshot(q("products"), s => {
        window.data.products = s.docs.map(d => ({ id: d.id, ...d.data() }));
        refreshCurrentView();
    }));
    listeners.push(onSnapshot(q("customers"), s => {
        window.data.customers = s.docs.map(d => ({ id: d.id, ...d.data() }));
        refreshCurrentView();
    }));
    listeners.push(onSnapshot(q("sales"), s => {
        window.data.sales = s.docs.map(d => ({ id: d.id, ...d.data() }));
        refreshCurrentView();
    }));
    listeners.push(onSnapshot(q("expenses"), s => {
        window.data.expenses = s.docs.map(d => ({ id: d.id, ...d.data() }));
        refreshCurrentView();
    }));
    listeners.push(onSnapshot(q("stockPurchases"), s => {
        window.data.stockPurchases = s.docs.map(d => ({ id: d.id, ...d.data() }));
        refreshCurrentView();
    }));
    listeners.push(onSnapshot(q("customerTransactions"), s => {
        window.data.customerTransactions = s.docs.map(d => ({ id: d.id, ...d.data() }));
        refreshCurrentView();
    }));
    listeners.push(onSnapshot(doc(db, "settings", window.currentUserId), (snap) => {
        window.data.settings = snap.exists() ? snap.data() : {};
        refreshCurrentView();
    }));
}
...
window.showSaleTab = showSaleTab;
window.renderCart = renderCart;
window.updateSaleDue = updateSaleDue;
window.addSaleItem = addSaleItem;
window.removeCartItem = removeCartItem;
window.completeNormalSale = completeNormalSale;
...
window.openDeleteRecordsModal = openDeleteRecordsModal;
window.deleteCollectionData = deleteCollectionData;
window.deleteEverything = deleteEverything;
