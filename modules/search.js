...
export function openGlobalSearchModal() {
    ...
    modal.innerHTML = `<div class="modal-header"><h2>Search</h2><button class="close-btn" onclick="closeModal()">&times;</button></div><div class="form-group"><input type="text" id="global-search-input" placeholder="Search products, customers, suppliers..." oninput="runGlobalSearch()" autofocus></div><div id="global-search-results"></div>`;
...
export function runGlobalSearch() {
    ...
    let html = '';
    if (products.length) html += `<h3 style="font-size:13px; color:var(--gray); margin:15px 0 5px;">PRODUCTS</h3>` + products.map(p => `<div class="list-item" onclick="closeModal(); navigate('inventory'); setTimeout(() => openProductModal('${p.id}'), 150);"><div class="list-item-info"><h4>${p.name}</h4><p>Stock: ${p.stock} | ${formatCurrency(p.price)}</p></div></div>`).join('');
    if (customers.length) html += `<h3 style="font-size:13px; color:var(--gray); margin:15px 0 5px;">CUSTOMERS</h3>` + customers.map(c => `<div class="list-item" onclick="closeModal(); setTimeout(() => openCustomerDetails('${c.id}'), 150);"><div class="list-item-info"><h4>${c.name}</h4><p>Debt: ${formatCurrency(c.balance || 0)}</p></div></div>`).join('');
    if (suppliers.length) html += `<h3 style="font-size:13px; color:var(--gray); margin:15px 0 5px;">SUPPLIERS</h3>` + suppliers.map(s => `<div class="list-item" onclick="closeModal(); setTimeout(() => openSupplierDetails('${s.id}'), 150);"><div class="list-item-info"><h4>${s.name}</h4><p>Payable: ${formatCurrency(s.balance || 0)}</p></div></div>`).join('');
