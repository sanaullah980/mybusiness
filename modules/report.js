...
export function renderReports(container) {
    ...
    const getStartOfMonth = window.getStartOfMonth;
    const getEndOfMonth = window.getEndOfMonth;
    let startDate, endDate;
    if (window.activeReportTab === 'daily') {
        startDate = getStartOfDay(new Date());
        endDate = getEndOfDay(new Date());
    } else if (window.activeReportTab === 'monthly') {
        startDate = getStartOfMonth(window.currentReportMonth);
        endDate = getEndOfMonth(window.currentReportMonth);
    } else {
    ...
    container.innerHTML = `
        ...
        ${window.activeReportTab === 'monthly' ? `
...
export function changeReportMonth(direction) {
    window.currentReportMonth.setMonth(window.currentReportMonth.getMonth() + direction);
    renderReports(document.getElementById('app-content'));
}
...
export async function resetDailyReport() {
    ...
    try {
        ...
        window.hideLoading('btn-reset-day');
    }
}