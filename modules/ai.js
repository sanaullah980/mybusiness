...
window.NativeWhisper._event=(kind,raw)=>{let d={};try{d=JSON.parse(raw||'{}')}catch(_){d={message:raw};}
 ...
 if(kind==='error'){voiceRecording=false;document.getElementById('ai-voice-btn')?.classList.remove('active');p?.classList.add('hidden');document.getElementById('whisper-card')?.classList.remove('hidden');if(s){s.textContent=d.message||'Voice error';s.dataset.state='error';}}
};
