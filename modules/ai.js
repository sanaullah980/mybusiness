...
function appendMessage(role,text,skipSave){
  ...
  box.appendChild(row);
  if(!skipSave){chatHistory.push({role,text});saveChatHistory();}
  scrollAIToBottom();
  return row;
}
function appendConfirmation(text){
  ...
  const finish=async(value)=>{yes.disabled=true;no.disabled=true;actions.remove();appendMessage('user',value);if(send)send.disabled=true;try{const result=await handleAgentCommand(value);if(result?.handled){if(result.confirmation)appendConfirmation(result.text);else appendMessage('assistant',result.text||'');}else appendMessage('assistant','Please try that command again.');}catch(e){appendMessage('assistant',`Agent error: ${e.message||e}`);}finally{if(send)send.disabled=false;scrollAIToBottom();}};
  yes.onclick=()=>finish('yes');no.onclick=()=>finish('no');
}
function finishAIRequest(d){const item=pending.get(d.id);if(!item)return;pending.delete(d.id);if(d.ok)appendMessage('assistant',d.text||'');else appendMessage('assistant',`Error: ${d.error||'Inference failed.'}`);const send=document.getElementById('ai-send');if(send)send.disabled=false;scrollAIToBottom();}
...
export function renderAI(container){
 ...
 if(chatHistory.length){const box=aiMessages();if(box){box.innerHTML='';chatHistory.forEach(m=>appendMessage(m.role,m.text,true));scrollAIToBottom();}}
