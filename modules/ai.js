...
window.NativeWhisper._event=(kind,raw)=>{let d={};try{d=JSON.parse(raw||'{}')}catch(_){d={message:raw};}
 ...
 if(kind==='install'){const pct=Number(d.percent);if(bar&&pct>=0)bar.style.width=pct+'%';if(txt)txt.textContent=pct>=0?`Installing Whisper base — ${pct}% • ${fmtMb(d.bytes)} / ${fmtMb(d.total)}`:`Installing Whisper base • ${fmtMb(d.bytes)}`;p?.classList.remove('hidden');return;}
 if(kind==='state'){if(s)s.textContent=d.message||d.status||'Whisper';if(d.status==='ready'){whisperReady=true;p?.classList.add('hidden');document.getElementById('whisper-card')?.classList.add('hidden');}else if(d.status==='installed'){whisperReady=false;}else if(d.status==='not_installed'){whisperReady=false;document.getElementById('whisper-card')?.classList.remove('hidden');}else if(d.status==='recording'){voiceRecording=true;document.getElementById('ai-voice-btn')?.classList.add('active');}else if(d.status==='transcribing'){voiceRecording=false;document.getElementById('ai-voice-btn')?.classList.remove('active');}updateCombinedAIAvailability();return;}
 if(kind==='result'){const text=String(d.text||'').trim();voiceRecording=false;document.getElementById('ai-voice-btn')?.classList.remove('active');if(text){const input=aiInput();if(input)input.value=text;sendAI();}return;}
 if(kind==='error'){voiceRecording=false;document.getElementById('ai-voice-btn')?.classList.remove('active');setAIError(d.message||'Voice error');}
};
