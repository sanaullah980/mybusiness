...
export async function completeNormalSale() { if(window.cart.length === 0) return alert("Cart is empty!"); const btn = document.getElementById('btn-complete-sale'); window.showLoading('btn-complete-sale', "Processing..."); try { const customerId = document.getElementById('sale-customer').value || null; const amountPaid = parseFloat(document.getElementById('sale-amount-paid').value) || 0; const discountType = document.getElementById('sale-discount-type').value || 'amount'; const discountValueInput = parseFloat(document.getElementById('sale-discount-value').value) || 0; const cartSnapshot = window.cart.map(i => ({ ...i })); await window.runAtomicOrOffline(async (transaction) => { const productSnapshots = []; for (const item of cartSnapshot) { const snap = await transaction.get(window.doc(window.db, "products", item.id)); productSnapshots.push({ item, snap }); } let customerSnap = null; if (customerId) customerSnap = await transaction.get(window.doc(window.db, "customers", customerId)); let subtotal = 0, rawProfit = 0; for (const { item, snap } of productSnapshots) { if (!snap.exists()) throw new Error(`Product ${item.name} not found.`); const pData = snap.data(); if (pData.ownerId !== window.currentUserId) throw new Error("Unauthorized."); if (pData.stock < item.qty) throw new Error(`Insufficient stock for ${item.name}.`); subtotal += item.price * item.qty; rawProfit += (item.price - pData.cost) * item.qty; } const rawDiscount = discountType === 'percent' ? subtotal * (discountValueInput / 100) : discountValueInput; const discount = Math.max(0, Math.min(subtotal, rawDiscount)); const total = subtotal - discount; const totalProfit = rawProfit - discount; if (amountPaid < 0 || amountPaid > total) throw new Error(`Amount paid cannot exceed the sale total of ${window.formatCurrency(total)}.`); const amountDue = Math.max(0, total - amountPaid); let newCustomerBalance = 0; if (customerId && amountDue > 0 && customerSnap && customerSnap.exists()) newCustomerBalance = (customerSnap.data().balance || 0) + amountDue; else if (customerId && customerSnap && customerSnap.exists()) newCustomerBalance = customerSnap.data().balance || 0; const saleRef = window.doc(window.collection(window.db, "sales")); const customerName = (customerId && customerSnap && customerSnap.exists()) ? customerSnap.data().name : "Walk-in"; const itemsWithReturnTracking = cartSnapshot.map(i => ({ ...i, returnedQty: 0 })); transaction.set(saleRef, { ownerId: window.currentUserId, customerId: customerId || null, customerName, saleType: 'normal', date: new Date().toISOString(), items: itemsWithReturnTracking, subtotal, discount, discountType, total, amountPaid, amountDue, totalProfit, profitKnown: true, note: '', returnedAmount: 0, returnedProfit: 0, invoiceNumber: null }); for (const { item, snap } of productSnapshots) transaction.update(window.doc(window.db, "products", item.id), { stock: snap.data().stock - item.qty }); if (customerId && amountDue > 0) { transaction.update(window.doc(window.db, "customers", customerId), { balance: newCustomerBalance }); const txnRef = window.doc(window.collection(window.db, "customerTransactions")); transaction.set(txnRef, { ownerId: window.currentUserId, customerId, type: 'sale_debt', amount: amountDue, balanceAfter: newCustomerBalance, date: new Date().toISOString(), note: `Sale #${saleRef.id.substring(0,8)}`, saleId: saleRef.id }); } }); alert("Sale completed!"); window.cart = []; window.navigate('dashboard'); } catch (error) { console.error(error); alert(error.message || "Unable to complete sale."); } finally { window.hideLoading('btn-complete-sale'); } }
