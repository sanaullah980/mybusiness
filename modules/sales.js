...
export function updateCartItemQty(index,value){const item=window.cart[index];if(!item)return;const product=(window.data.products||[]).find(p=>p.id===item.id);const maxQty=product?Math.max(1,Number(product.stock)||0):Number.MAX_SAFE_INTEGER;let qty=Math.max(1,parseInt(value,10)||1);if(product&&qty>maxQty){window.showToast?.(`Only ${maxQty} in stock for ${item.name}.`,'warning');qty=maxQty;}item.qty=qty;renderCart();}
...
export async function completeNormalSale() {
    ...
    try{
      const customerId=document.getElementById('sale-customer').value||null, amountPaid=parseFloat(document.getElementById('sale-amount-paid').value)||0, discountType=document.getElementById('sale-discount-type').value||'amount', discountValueInput=parseFloat(document.getElementById('sale-discount-value').value)||0, cartSnapshot=window.cart.map(i=>({...i}));
      await window.runAtomicOrOffline(async transaction=>{
        ...
        for(const {item,snap} of productSnapshots){if(!snap.exists())throw new Error(`Product ${item.name} not found.`);const p=snap.data();if(p.ownerId!==window.currentUserId)throw new Error('Unauthorized.');if(Number(p.stock||0)<item.qty)throw new Error(`Insufficient stock for ${item.name}. Available: ${Number(p.stock||0)}.`);const unit=saleType==='retail'?Number(p.retailPrice??p.price):Number(p.wholesalePrice??p.price);item.price=unit;subtotal+=unit*item.qty;rawProfit+=(unit-Number(p.cost||0))*item.qty;}
        const rawDiscount=discountType==='percent'?subtotal*(discountValueInput/100):discountValueInput,discount=Math.max(0,Math.min(subtotal,rawDiscount)),total=subtotal-discount,totalProfit=rawProfit-discount;
        if(amountPaid<0)throw new Error('Amount paid cannot be negative.');
        const amountDue=Math.max(0,total-amountPaid),current=customerSnap?.exists()?Math.max(0,Number(customerSnap.data().balance||0)):0,newCustomerBalance=Math.max(0,current+total-amountPaid),saleRef=window.doc(window.collection(window.db,'sales')),customerName=customerSnap?.exists()?customerSnap.data().name:'Walk-in';
        transaction.set(saleRef,{ownerId:window.currentUserId,createdBy:window.authUserId||window.currentUserId,createdByName:window.currentMemberName||'Business Owner',customerId:customerId||null,customerName,saleType,date:new Date().toISOString(),items:cartSnapshot.map(i=>({...i,returnedQty:0})),subtotal,discount,discountType,total,amountPaid,amountDue,totalProfit,profitKnown:true,note:'',returnedAmount:0,returnedProfit:0,invoiceNumber:null});
        ...
        if(customerId){transaction.update(window.doc(window.db,'customers',customerId),{balance:newCustomerBalance,updatedAt:new Date().toISOString()});const txnRef=window.doc(window.collection(window.db,'customerTransactions'));transaction.set(txnRef,{ownerId:window.currentUserId,customerId,type:'sale_debt',amount:total,amountPaid:amountPaid,debitAmount:total,creditAmount:amountPaid,balanceAfter:newCustomerBalance,date:new Date().toISOString(),note:`Bill No. ${saleRef.id.substring(0,8)}`,billNo:saleRef.id.substring(0,8),saleId:saleRef.id,createdBy:window.authUserId||window.currentUserId});}
      });
