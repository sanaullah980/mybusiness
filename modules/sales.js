...
export function updateSaleDue() {
    const subtotal=window.cart.reduce((sum,item)=>sum+(Number(item.price)||0)*(Number(item.qty)||0),0);
    const type=window.activeSaleType==='retail'?'retail':'wholesale';
    const discountType=document.getElementById('sale-discount-type')?.value||'amount';
    const discountValueInput=parseFloat(document.getElementById('sale-discount-value')?.value)||0;
    const requestedDiscount=discountType==='percent'?subtotal*(discountValueInput/100):discountValueInput;
    const baseDiscount=Math.max(0,Math.min(subtotal,requestedDiscount));
    const beforePaidTotal=Math.max(0,subtotal-baseDiscount);
    let paid=parseFloat(document.getElementById('sale-amount-paid')?.value)||0;
    paid=Math.max(0,Math.min(beforePaidTotal,paid));
    // Retail sales never create debt. Anything the customer does not pay is treated as an
    // additional discount, so the final payable total exactly equals the amount paid.
    const automaticRetailDiscount=type==='retail'?Math.max(0,beforePaidTotal-paid):0;
    const finalDiscount=baseDiscount+automaticRetailDiscount;
    const total=Math.max(0,subtotal-finalDiscount);
    const due=type==='retail'?0:Math.max(0,total-paid);
    ...
    if(document.getElementById('sale-amount-due'))document.getElementById('sale-amount-due').value=window.formatCurrency(type==='retail'?automaticRetailDiscount:due);
}
...
export async function completeNormalSale() {
    ...
    try{
      ...
      await window.runAtomicOrOffline(async transaction=>{
        ...
        const baseDiscount=Math.max(0,Math.min(subtotal,rawDiscount));
        if(amountPaid<0)throw new Error('Amount paid cannot be negative.');
        const beforePaidTotal=Math.max(0,subtotal-baseDiscount);
        if(amountPaid>beforePaidTotal)throw new Error(`Amount paid cannot exceed ${window.formatCurrency(beforePaidTotal)}.`);
        const automaticRetailDiscount=saleType==='retail'?Math.max(0,beforePaidTotal-amountPaid):0;
        const discount=Math.min(subtotal,baseDiscount+automaticRetailDiscount);
        ...
        const newCustomerBalance=Math.max(0,current+total-amountPaid);
        const saleRef=window.doc(window.collection(window.db,'sales')),customerName=customerSnap?.exists()?customerSnap.data().name:'Walk-in';
        ...
        if(customerId&&saleType!=='retail'&&amountDue>0){
