...
export async function completeNormalSale() { if(!window.cart.length)return alert('Cart is empty!'); const saleType=window.activeSaleType==='retail'?'retail':'wholesale'; window.showLoading('btn-complete-sale','Processing...'); try{const customerId=document.getElementById('sale-customer').value||null, amountPaid=parseFloat(document.getElementById('sale-amount-paid').value)||0, discountType=document.getElementById('sale-discount-type').value||'amount', discountValueInput=parseFloat(document.getElementById('sale-discount-value').value)||0, cartSnapshot=window.cart.map(i=>({...i})); await window.runAtomicOrOffline(async transaction=>{const productDocs=cartSnapshot.map(i=>window.doc(window.db,'products',i.id));const refs=customerId?[...productDocs,window.doc(window.db,'customers',customerId)]:productDocs;const snapshots=await Promise.all(refs.map(ref=>transaction.get(ref)));const productSnapshots=cartSnapshot.map((item,index)=>({item,snap:snapshots[index]}));const customerSnap=customerId?snapshots.at(-1):null;let subtotal=0,rawProfit=0;for(const {item,snap} of productSnapshots){if(!snap.exists())throw new Error(`Product ${item.name} not found.`);const p=snap.data();if(p.ownerId!==window.currentUserId)throw new Error('Unauthorized.');if((p.stock||0)<item.qty)throw new Error(`Insufficient stock for ${item.name}.`);const unit=saleType==='retail'?Number(p.retailPrice??p.price):Number(p.wholesalePrice??p.price);item.price=unit;subtotal+=unit*item.qty;rawProfit+=(unit-Number(p.cost||0))*item.qty;}const rawDiscount=discountType==='percent'?subtotal*(discountValueInput/100):discountValueInput;const discount=Math.max(0,Math.min(subtotal,rawDiscount));const total=subtotal-discount,totalProfit=rawProfit-discount;if(amountPaid<0||amountPaid>total)throw new Error(`Amount paid cannot exceed ${window.formatCurrency(total)}.`);const amountDue=Math.max(0,total-amountPaid);const current=customerSnap?.exists()?Number(customerSnap.data().balance||0):0;const newCustomerBalance=current+amountDue;const saleRef=window.doc(window.collection(window.db,'sales'));const customerName=customerSnap?.exists()?customerSnap.data().name:'Walk-in';transaction.set(saleRef,{ownerId:window.currentUserId,createdBy:window.authUserId||window.currentUserId,createdByName:window.currentMemberName||'Business Owner',customerId:customerId||null,customerName,saleType,date:new Date().toISOString(),items:cartSnapshot.map(i=>({...i,returnedQty:0})),subtotal,discount,discountType,total,amountPaid,amountDue,totalProfit,profitKnown:true,note:'',returnedAmount:0,returnedProfit:0,invoiceNumber:null});for(const {item,snap} of productSnapshots)transaction.update(window.doc(window.db,'products',item.id),{stock:Number(snap.data().stock||0)-item.qty});if(customerId&&amountDue>0){transaction.update(window.doc(window.db,'customers',customerId),{balance:newCustomerBalance,updatedAt:new Date().toISOString()});const txnRef=window.doc(window.collection(window.db,'customerTransactions'));transaction.set(txnRef,{ownerId:window.currentUserId,customerId,type:'sale_debt',amount:amountDue,balanceAfter:newCustomerBalance,date:new Date().toISOString(),note:`Bill No. ${saleRef.id.substring(0,8)}`,billNo:saleRef.id.substring(0,8),saleId:saleRef.id});}});window.showToast?.(`${saleType==='retail'?'Retail':'Wholesale'} sale completed!`,'success');window.cart=[];window.navigate('dashboard');}catch(e){console.error(e);alert(e.message||'Unable to complete sale.');}finally{window.hideLoading('btn-complete-sale');}}
...
export async function completeManualSale() {
    ...
    try{
        ...
        await window.runAtomicOrOffline(async transaction=>{
            ...
            transaction.set(saleRef,{ownerId:window.currentUserId,createdBy:window.authUserId||window.currentUserId,createdByName:window.currentMemberName||'Business Owner',customerId,customerName,saleType:'manual',date:new Date().toISOString(),items:[{name,price:amount,cost:amount-totalProfit,qty:1}],total:amount,amountPaid,amountDue,totalProfit,profitKnown,note:'Manual entry'});
