...
export function openProductSelectionModal() {
    ...
    const productsListHtml = window.data.products.map(p => `<div class="product-select-item-wrapper" data-name="${p.name.toLowerCase()}" style="border-bottom:1px solid #eee;"><div class="product-select-item" onclick="toggleProductRow('${p.id}')" style="display:flex; align-items:center; padding:15px 10px; cursor:pointer;"><input type="checkbox" id="chk-${p.id}" style="margin-right:15px; transform: scale(1.5); pointer-events: none;"><div style="flex:1;"><div style="font-weight:bold; font-size:16px;">${p.name}</div><div style="font-size:13px; color:var(--gray);">Stock: ${p.stock} | Price: ${window.formatCurrency(p.price)}</div></div><i class="fas fa-chevron-right" style="color:var(--gray);"></i></div><div id="qty-container-${p.id}" class="hidden" style="padding:10px 15px 15px 45px; background:#f8f9fa;"><label style="font-size:14px; font-weight:bold; color:var(--dark);">Quantity:</label><input type="number" id="qty-${p.id}" value="1" min="1" max="${p.stock}" style="width:100px; padding:8px; margin-left:10px; border:1px solid #ddd; border-radius:4px; font-size:16px;"></div></div>`).join('');
    modal.innerHTML = `<div class="modal-header"><h2>Select Products</h2><button class="close-btn" onclick="closeModal()">&times;</button></div><div class="form-group" style="position:sticky; top:0; background:white; padding-bottom:10px; z-index:10;"><input type="text" id="product-search" placeholder="Search products..." style="width:100%; padding:12px; border:1px solid #ddd; border-radius:8px; font-size:16px;" oninput="filterProductSelectionList()"></div><div id="product-selection-list" style="max-height: 50vh; overflow-y: auto;">${productsListHtml.length > 0 ? productsListHtml : '<p style="text-align:center; padding:20px; color:var(--gray);">No products found. Add products in Inventory first.</p>'}</div><div style="margin-top: 15px;"><button class="btn" onclick="addSelectedProductsToCart()">Add Selected to Cart</button></div>`;
    document.getElementById('modal-overlay').classList.remove('hidden');
}

export function toggleProductRow(id) {
    const checkbox = document.getElementById(`chk-${id}`); checkbox.checked = !checkbox.checked;
    const qtyContainer = document.getElementById(`qty-container-${id}`);
    if (checkbox.checked) { qtyContainer.classList.remove('hidden'); setTimeout(() => document.getElementById(`qty-${id}`).focus(), 10); } 
    else { qtyContainer.classList.add('hidden'); }
}
...
export function filterProductSelectionList() {
    ...
    items.forEach(wrapper => { const name = wrapper.dataset.name; if (name.includes(searchTerm)) { wrapper.classList.remove('hidden'); } else { wrapper.classList.add('hidden'); } });
}

export function addSelectedProductsToCart() {
    let addedCount = 0; let errorMessage = "";
    window.data.products.forEach(p => {
        const checkbox = document.getElementById(`chk-${p.id}`);
        if (checkbox && checkbox.checked) {
            const qtyInput = document.getElementById(`qty-${p.id}`); const qty = parseInt(qtyInput.value);
            if (!qty || qty < 1) { errorMessage += `Please enter a valid quantity for ${p.name}.\n`; return; }
            if (qty > p.stock) { errorMessage += `Not enough stock for ${p.name}! Available: ${p.stock}\n`; return; }
            const existing = window.cart.find(i => i.id === p.id);
            if (existing) { if (existing.qty + qty > p.stock) { errorMessage += `Not enough stock for ${p.name}! Cart already has ${existing.qty}, available: ${p.stock}\n`; return; } existing.qty += qty; } 
            else { const item = { id: p.id, name: p.name, price: p.price, cost: p.cost, qty: qty }; window.cart.push(item); }
            addedCount++; checkbox.checked = false; document.getElementById(`qty-container-${p.id}`).classList.add('hidden'); qtyInput.value = 1;
        }
    });
    if (errorMessage) { alert(errorMessage); } else if (addedCount > 0) { renderCart(); closeModal(); } else { alert("Please select at least one product."); }
}