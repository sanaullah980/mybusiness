...
export function updateSaleDue() {
    const subtotal = window.cart.reduce((sum, item) => sum + (Number(item.price) * Number(item.qty)), 0);
    const saleType = window.activeSaleType === 'retail' ? 'retail' : 'wholesale';
    const discountType = document.getElementById('sale-discount-type')?.value || 'amount';
    const discountValueInput = Math.max(0, parseFloat(document.getElementById('sale-discount-value')?.value) || 0);
    const rawDiscount = discountType === 'percent' ? subtotal * (discountValueInput / 100) : discountValueInput;
    const enteredDiscount = Math.max(0, Math.min(subtotal, rawDiscount));
    const afterEnteredDiscount = Math.max(0, subtotal - enteredDiscount);
    const paid = Math.max(0, parseFloat(document.getElementById('sale-amount-paid')?.value) || 0);
    // Retail sales never create customer debt. If the customer pays less than the
    // discounted total, the difference is an additional discount on this bill.
    const retailRemainder = saleType === 'retail' ? Math.max(0, afterEnteredDiscount - paid) : 0;
    const effectiveDiscount = saleType === 'retail' ? Math.min(subtotal, enteredDiscount + retailRemainder) : enteredDiscount;
    const total = Math.max(0, subtotal - effectiveDiscount);
    const due = saleType === 'retail' ? 0 : Math.max(0, total - paid);
    if (document.getElementById('cart-subtotal-display')) document.getElementById('cart-subtotal-display').value = window.formatCurrency(subtotal);
    if (document.getElementById('cart-total-display')) document.getElementById('cart-total-display').value = window.formatCurrency(total);
    if (document.getElementById('sale-amount-due')) document.getElementById('sale-amount-due').value = window.formatCurrency(saleType === 'retail' ? retailRemainder : due);
}
...
export async function completeNormalSale() {
    ...
    try{
      ...
      const billNumber=Math.max(1,parseInt(document.getElementById('sale-bill-number')?.value,10)||getNextBillNumber());
      const amountPaid=Math.max(0,parseFloat(document.getElementById('sale-amount-paid').value)||0);
      const discountType=document.getElementById('sale-discount-type').value||'amount';
      const discountValueInput=Math.max(0,parseFloat(document.getElementById('sale-discount-value').value)||0);
      const cartSnapshot=window.cart.map(i=>({...i}));
      await window.runAtomicOrOffline(async transaction=>{
        const productDocs=cartSnapshot.map(i=>window.doc(window.db,'products',i.id));
        // Retail sales may still be associated with a customer for history, but they
        // never change that customer's debt balance.
        const refs=saleType==='wholesale'&&customerId?[...productDocs,window.doc(window.db,'customers',customerId)]:productDocs;
        const snapshots=await Promise.all(refs.map(ref=>transaction.get(ref)));
        const productSnapshots=cartSnapshot.map((item,index)=>({item,snap:snapshots[index]}));
        const customerSnap=saleType==='wholesale'&&customerId?snapshots.at(-1):null;
        ...
        for(const {item,snap} of productSnapshots){
          ...
          const p=snap.data();
          if(p.ownerId!==window.currentUserId)throw new Error('Unauthorized.');
          const unit=saleType==='retail'?Number(p.retailPrice??p.price??0):Number(p.wholesalePrice??p.price??0);
          item.price=unit;
          subtotal+=unit*Number(item.qty||0);
          rawProfit+=(unit-Number(p.cost||0))*Number(item.qty||0);
        }
        const rawDiscount=discountType==='percent'?subtotal*(discountValueInput/100):discountValueInput;
        const enteredDiscount=Math.max(0,Math.min(subtotal,rawDiscount));
        const afterEnteredDiscount=Math.max(0,subtotal-enteredDiscount);
        if(amountPaid>subtotal)throw new Error(`Amount paid cannot exceed ${window.formatCurrency(subtotal)}.`);
        const retailRemainder=saleType==='retail'?Math.max(0,afterEnteredDiscount-amountPaid):0;
        const discount=saleType==='retail'?Math.min(subtotal,enteredDiscount+retailRemainder):enteredDiscount;
        const total=Math.max(0,subtotal-discount);
        const totalProfit=rawProfit-discount;
        const amountDue=saleType==='retail'?0:Math.max(0,total-amountPaid);
        const current=customerSnap?.exists()?Math.max(0,Number(customerSnap.data().balance||0)):0;
        const newCustomerBalance=Math.max(0,current+amountDue);
        const saleRef=window.doc(window.collection(window.db,'sales'));
        const customerName=(saleType==='wholesale'&&customerSnap?.exists())?customerSnap.data().name:(customerId?(window.data.customers||[]).find(c=>c.id===customerId)?.name||'Customer':'Walk-in');
        ...
        for(const {item,snap} of productSnapshots){
          ...
          if(Number(item.qty)>available)throw new Error(`Not enough stock for ${item.name}. Available: ${available}.`);
          transaction.update(window.doc(window.db,'products',item.id),{stock:available-Number(item.qty)});
        }
        if(saleType==='wholesale'&&customerId&&amountDue>0){
          transaction.update(window.doc(window.db,'customers',customerId),{balance:newCustomerBalance,updatedAt:new Date().toISOString()});
          const txnRef=window.doc(window.collection(window.db,'customerTransactions'));
          transaction.set(txnRef,{ownerId:window.currentUserId,customerId,type:'sale_debt',amount:amountDue,amountPaid:amountPaid,debitAmount:amountDue,creditAmount:0,balanceAfter:newCustomerBalance,date:new Date().toISOString(),note:`Bill No. ${billNumber}`,billNo:String(billNumber),saleId:saleRef.id,createdBy:window.authUserId||window.currentUserId});
        }
