...
export async function completeManualSale() {
    ...
    try{
        ...
        await window.runAtomicOrOffline(async transaction=>{
            ...
            transaction.set(saleRef,{ownerId:window.currentUserId,createdBy:window.authUserId||window.currentUserId,customerId,customerName,saleType:'manual',date:new Date().toISOString(),items:[{name,price:amount,cost:amount-totalProfit,qty:1}],total:amount,amountPaid,amountDue,totalProfit,profitKnown,note:'Manual entry'});
