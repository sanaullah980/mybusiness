...
export function updateSaleDue() { const subtotal = window.cart.reduce((sum, item) => sum + (item.price * item.qty), 0); const saleType = window.activeSaleType === 'retail' ? 'retail' : 'wholesale'; const discountType = document.getElementById('sale-discount-type')?.value || 'amount'; const discountValueInput = Math.max(0, parseFloat(document.getElementById('sale-discount-value')?.value) || 0); const rawDiscount = discountType === 'percent' ? subtotal * (discountValueInput / 100) : discountValueInput; const enteredDiscount = Math.max(0, Math.min(subtotal, rawDiscount)); const afterEnteredDiscount = Math.max(0, subtotal - enteredDiscount); const paid = Math.max(0, parseFloat(document.getElementById('sale-amount-paid')?.value) || 0); const retailRemainder = saleType === 'retail' ? Math.max(0, afterEnteredDiscount - paid) : 0; const effectiveDiscount = saleType === 'retail' ? Math.min(subtotal, enteredDiscount + retailRemainder) : enteredDiscount; const total = Math.max(0, subtotal - effectiveDiscount); const due = saleType === 'retail' ? retailRemainder : Math.max(0, total - paid); if (document.getElementById('cart-subtotal-display')) document.getElementById('cart-subtotal-display').value = window.formatCurrency(subtotal); if (document.getElementById('cart-total-display')) document.getElementById('cart-total-display').value = window.formatCurrency(total); if (document.getElementById('sale-amount-due')) document.getElementById('sale-amount-due').value = window.formatCurrency(due); }
...
export async function completeNormalSale() {
    ...
    try{
      const customerId=document.getElementById('sale-customer').value||null, billNumber=Math.max(1,parseInt(document.getElementById('sale-bill-number')?.value,10)||getNextBillNumber()), amountPaidInput=Math.max(0,parseFloat(document.getElementById('sale-amount-paid').value)||0), discountType=document.getElementById('sale-discount-type').value||'amount', discountValueInput=Math.max(0,parseFloat(document.getElementById('sale-discount-value').value)||0), cartSnapshot=window.cart.map(i=>({...i}));
      await window.runAtomicOrOffline(async transaction=>{
        ...
        if(amountPaidInput>subtotal)throw new Error(`Amount paid cannot be greater than ${window.formatCurrency(subtotal)}.`);
        ...
        // Retail has no debt. Any unpaid remainder is automatically converted to an additional discount.
        const retailRemainder=saleType==='retail'?Math.max(0,subtotal-enteredDiscount-amountPaidInput):0;
        const discount=saleType==='retail'?Math.min(subtotal,enteredDiscount+retailRemainder):enteredDiscount;
        const total=Math.max(0,subtotal-discount);
        const amountPaid=saleType==='retail'?total:amountPaidInput;
        ...
        if(customerSnap?.exists()&&customerSnap.data().ownerId!==window.currentUserId)throw new Error('Unauthorized customer.');
