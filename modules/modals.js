...
export function viewSaleDetail(saleId) {
    const sale = window.data.sales.find(s => s.id === saleId);
    if (!sale) return;
    const formatCurrency = window.formatCurrency;
    let debtHtml = '';
    if (sale.customerId) {
        const customer = window.data.customers.find(c => c.id === sale.customerId);
        const currentDebt = customer ? (customer.balance || 0) : 0;
        debtHtml = `<div style="background:#fff3e0; border-left:4px solid var(--warning); padding:12px; border-radius:6px; margin:15px 0;">
            ...
            <div style="font-size:13px; margin-top:5px;">Amount due from this sale: ${formatCurrency(sale.amountDue || 0)}</div>
        </div>`;
    }
    ...
    modal.innerHTML = `
        ...
        <div style="background:#f8f9fa; padding:15px; border-radius:8px; margin-bottom:15px;">
            <div style="display:flex; justify-content:space-between; margin-bottom:10px;">
                <div>
                    <div style="font-size:12px; color:var(--gray);">Date & Time</div>
                    <div style="font-size:16px; font-weight:bold;">${new Date(sale.date).toLocaleString()}</div>
                </div>
                <div style="text-align:right;">
                    <div style="font-size:12px; color:var(--gray);">Customer</div>
                    <div style="font-size:16px; font-weight:bold;">${sale.customerName || 'Walk-in'}</div>
                </div>
            </div>
            <div style="display:flex; justify-content:space-between;">
                <div>
                    <div style="font-size:12px; color:var(--gray);">Sale Type</div>
                    <div style="font-size:16px; font-weight:bold;">${sale.saleType ? sale.saleType.charAt(0).toUpperCase() + sale.saleType.slice(1) : 'Normal'}</div>
                </div>
                <div style="text-align:right;">
                    <div style="font-size:12px; color:var(--gray);">Profit Status</div>
                    <div style="font-size:16px; font-weight:bold; color:${sale.profitKnown ? 'var(--primary)' : 'var(--warning)'};">${sale.profitKnown ? 'Known' : 'Unknown'}</div>
                </div>
            </div>
        </div>
        ${debtHtml}
