export function renderExpenses(container) {
    ...
    const getLocalDateStr = window.getLocalDateStr;
...
export function openExpenseModal(expenseId = null) {
    const e = expenseId ? window.data.expenses.find(x => x.id === expenseId) : null;
    ...
    modal.innerHTML = `
        ...
        <div class="form-group"><label>Date *</label><input type="date" id="exp-date" value="${e ? window.getLocalDateStr(e.date) : window.getLocalDateStr(new Date())}"></div>
        <div class="form-group"><label>Category</label><input type="text" id="exp-category" value="${e ? (e.category || '') : ''}"></div>
        <div class="form-group"><label>Note</label><input type="text" id="exp-note" value="${e ? (e.note || '') : ''}"></div>
        <button class="btn" id="btn-save-expense" onclick="saveExpense('${expenseId || ''}')">${e ? 'Update' : 'Save'} Expense</button>
    `;
    document.getElementById('modal-overlay').classList.remove('hidden');
}

export async function saveExpense(expenseId) {
    const amount = parseFloat(document.getElementById('exp-amount').value);
    const date = document.getElementById('exp-date').value;
    const category = document.getElementById('exp-category').value.trim();
    const note = document.getElementById('exp-note').value.trim();
    if (isNaN(amount) || amount <= 0 || !date) return alert("Valid amount and date.");
    window.showLoading('btn-save-expense', "Saving...");
    try {
        const eData = { amount, date: new Date(date).toISOString(), category, note, ownerId: window.currentUserId };
        if (expenseId) await window.updateDoc(window.doc(window.db, "expenses", expenseId), eData);
        else await window.addDoc(window.collection(window.db, "expenses"), eData);
        alert(expenseId ? "Updated!" : "Added!");
        closeModal();
    } catch (error) {
        alert("Failed.");
    } finally {
        window.hideLoading('btn-save-expense');
    }
}
