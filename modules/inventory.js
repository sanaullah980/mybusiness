...
export function openProductModal(productId = null) {
    const p = productId ? window.data.products.find(x => x.id === productId) : null;
...
export async function saveProduct(productId) {
    ...
    if (!name || isNaN(cost) || isNaN(price) || isNaN(stock)) return alert("Fill all fields.");
    if (cost < 0 || price < 0 || stock < 0) return alert("No negative values.");
    window.showLoading('btn-save-product', "Saving...");
    try {
        const pData = { name, cost, price, stock, minStock, ownerId: window.currentUserId };
        if (productId) await window.updateDoc(window.doc(window.db, "products", productId), pData);
        else await window.addDoc(window.collection(window.db, "products"), pData);
        alert(productId ? "Updated!" : "Added!");
        closeModal();
    } catch (error) {
        console.error(error);
        alert("Failed.");
    } finally {
        window.hideLoading('btn-save-product');
    }
}
...
export function openStockAdjustModal(productId) {
    ...
    modal.innerHTML = `
        ...
        <div class="form-group"><label>Note</label><input type="text" id="adj-note"></div>
...
export async function saveStockAdjustment(productId) {
    ...
    if (!qty || qty <= 0) return alert("Valid qty.");
    const p = window.data.products.find(x => x.id === productId);
    if (type === 'remove' && p.stock - qty < 0) return alert("Cannot go below zero.");
    window.showLoading('btn-save-adj', "Updating...");
    try {
        const newStock = type === 'add' ? p.stock + qty : p.stock - qty;
        await window.updateDoc(window.doc(window.db, "products", productId), { stock: newStock });
        await window.addDoc(window.collection(window.db, "stockAdjustments"), {
            ownerId: window.currentUserId, productId, type, quantity: qty,
        ...
        window.hideLoading('btn-save-adj');
    }
}