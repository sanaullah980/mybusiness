export function renderInventory(container) { const data = window.data; const formatCurrency = window.formatCurrency; const esc = window.esc; container.innerHTML = `<button class="btn admin-only" style="margin-bottom:20px;" onclick="openProductModal()">+ Add Product</button><div class="card" id="product-list">${data.products.length === 0 ? '<div class="empty-state"><i class="fas fa-box-open"></i><h3>No products yet</h3><p>Add your first product to start tracking stock and sales.</p></div>' : data.products.map(p => `<div class="list-item" style="cursor:default;"> <div class="list-item-info"> <h4>${esc(p.name)}</h4> <p>Cost: ${formatCurrency(p.cost)} | Price: ${formatCurrency(p.price)}</p> <p>Stock: ${p.stock}${p.barcode ? ' | Barcode: ' + esc(p.barcode) : ''}</p> </div> <div style="display:flex; flex-direction:column; gap:8px; align-items:flex-end;"> <span class="badge ${p.stock <= (p.minStock||5) ? 'badge-low' : 'badge-ok'}">${p.stock <= (p.minStock||5) ? 'Low Stock' : 'OK'}</span> <div style="display:flex; gap:5px;"> <button class="btn btn-sm btn-secondary" onclick="openStockAdjustModal('${p.id}')">Adjust</button> <button class="btn btn-sm admin-only" onclick="openProductModal('${p.id}')">Edit</button> <button class="btn btn-sm btn-danger admin-only" onclick="deleteProduct('${p.id}')">Delete</button> </div> </div> </div>`).join('')}</div>`; }
export function openProductModal(productId = null) { const p = productId ? window.data.products.find(x => x.id === productId) : null; const esc = window.esc; const modal = document.getElementById('modal-body'); modal.innerHTML = `<div class="modal-header"><h2>${p ? 'Edit' : 'Add'} Product</h2><button class="close-btn" onclick="closeModal()">&times;</button></div><div class="form-group"><label>Product Name *</label><input type="text" id="p-name" value="${p ? esc(p.name) : ''}"></div><div class="form-group"><label>Barcode (Optional)</label><input type="text" id="p-barcode" value="${p ? esc(p.barcode || '') : ''}" placeholder="Scan with a barcode scanner or type manually"></div><div class="form-row"><div class="form-group"><label>Cost Price (Rs.) *</label><input type="number" id="p-cost" step="0.01" min="0" value="${p ? p.cost : ''}"></div><div class="form-group"><label>Selling Price (Rs.) *</label><input type="number" id="p-price" step="0.01" min="0" value="${p ? p.price : ''}"></div></div><div class="form-row"><div class="form-group"><label>Current Stock *</label><input type="number" id="p-stock" min="0" value="${p ? p.stock : ''}"></div><div class="form-group"><label>Min Stock Alert</label><input type="number" id="p-min" min="0" value="${p ? (p.minStock||5) : 5}"></div></div><button class="btn" id="btn-save-product" onclick="saveProduct('${productId || ''}')">${p ? 'Update' : 'Save'} Product</button>`; document.getElementById('modal-overlay').classList.remove('hidden'); }
...
export async function deleteProduct(id) { if (!confirm("Delete?")) return; try { await window.deleteDoc(window.doc(window.db, "products", id)); } catch (error) { alert("Failed."); } }
export function openStockAdjustModal(productId) { const p = window.data.products.find(x => x.id === productId); const esc = window.esc; const modal = document.getElementById('modal-body'); modal.innerHTML = `<div class="modal-header"><h2>Adjust Stock: ${esc(p.name)}</h2><button class="close-btn" onclick="closeModal()">&times;</button></div><p style="margin-bottom:15px; color:var(--gray);">Current Stock: <strong>${p.stock}</strong></p><div class="form-group"><label>Action</label><select id="adj-type"><option value="add">Add Stock</option><option value="remove">Remove Stock</option></select></div><div class="form-group"><label>Quantity</label><input type="number" id="adj-qty" min="1" value="1"></div><div class="form-group"><label>Note</label><input type="text" id="adj-note"></div><button class="btn" id="btn-save-adj" onclick="saveStockAdjustment('${productId}')">Update Stock</button>`; document.getElementById('modal-overlay').classList.remove('hidden'); }
